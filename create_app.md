...
Note that there is only one possible input and output object type: `move::moveStack` in `rds` file format. You can use the 4 example data sets (`input1_pigeons.rds`, `input2_geese.rds`, `input3_stork.rds`, `input4_goat.rds`) provided in the GitHub templates. The `logger.R` file is included in the SDKs for communication functionality functions in MoveApps. In this file, functions are defined that can be used by the App developer to give information messages, warnings, errors, etc. to the user in MoveApps. Please use these functions in your Apps.
